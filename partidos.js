...
function gerarComandosSQL() {
    ...
    fs.readFile('./2024/partidos_modificado.txt', 'utf8', (err, data) => {
        if (err) {
        ...
        const gravarArquivo = (comandos) => {
            return new Promise((resolve, reject) => {
                ...
                const arquivoPath = path.join(pastaResultado, 'partidos.sql'); // Nome do arquivo único
        ...
        const processarLinhas = async () => {
            for (const linha of linhas) {
                ...
                const cgmf = colunas[0].replace("201", ""); // Primeiro elemento
                const nome = colunas.slice(1).join(' ').trim().replaceAll("'", ""); // Juntar os elementos do terceiro em diante
